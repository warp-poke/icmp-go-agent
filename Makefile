...
DFLAGS=-race
CFLAGS=-X github.com/warp-poke/icmp-go-agent/cmd.githash=$(GITHASH)
...
build: poke-icmp-agent.go $$(call rwildcard, ./cmd, *.go) $$(call rwildcard, ./core, *.go)
...
.PHONY: format
format:
	gofmt -w -s ./cmd ./core ./models poke-icmp-agent.go
